...
public class NotificationService {
    ...
    public void generateReportAndSendMail() {
        ...
        for (User user : allUsers) {
            List<Training> recentTrainings = trainingProvider.findTrainingByUser(user.getId()).stream().filter(training -> toLocalDateTime(training.getStartTime()).isAfter(oneWeekAgo)).collect(Collectors.toList());
            if (!recentTrainings.isEmpty()) {
                final EmailDto emailDto = emailProvider.sendMail(user.getEmail(), reportString, recentTrainings);
