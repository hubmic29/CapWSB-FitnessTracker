...
interface TrainingRepository extends JpaRepository<Training, Long> {
    ...
    default List<Training> findByUserObject(User user) {
        return findAll().stream()
                .filter(training -> Objects.equals(training.getUser(), user)).collect(Collectors.toList());
    }

    default List<Training> findByEndTimeAfter(Date afterTime) {
        return findAll().stream().filter(
                training -> Objects.compare(training.getEndTime(), afterTime, Comparator.naturalOrder()) > 0).collect(Collectors.toList());
    }

    default List<Training> findByActivityType(ActivityType activityType){
        return findAll().stream().filter(
                training -> Objects.equals(training.getActivityType(), activityType)).collect(Collectors.toList());
    }
}
